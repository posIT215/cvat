    ...
    public get contours(): Contours {
        ...
        return {
            ...
            fitting: (points: number[] , threshold: number, imageData : any,closed = true): number[] => {
                ...
                const region = binaryMat.roi(new cv.Rect(x1, y1, x2 - x1, y2 - y1));
                const fittedBoxes: number[][] = [];
                // 텍스트가 있는 가장자리를 찾기
                const rows = new cv.Mat();
                cv.reduce(region, rows, 0, cv.REDUCE_MAX, cv.CV_8U);
                const cols = new cv.Mat();
                cv.reduce(region, cols, 1, cv.REDUCE_MAX, cv.CV_8U);

                const rowsData = new Uint8Array(rows.data);
                const colsData = new Uint8Array(cols.data);

                const rowsIndices: number[] = [];
                const colsIndices: number[] = [];
                ...
                if (rowsIndices.length > 0 && colsIndices.length > 0) {
                    const x1New = rowsIndices[0];
                    const x2New = rowsIndices[rowsIndices.length - 1];
                    const y1New = colsIndices[0];
                    const y2New = colsIndices[colsIndices.length - 1];

                    // 너무 딱 붙으면 짤리기 때문에, 적당히 박스를 띄어줌
                    const fittedX1 = x1 + x1New - 2;
                    const fittedY1 = y1 + y1New - 2;
                    const fittedX2 = x1 + x2New + 2;
                    const fittedY2 = y1 + y2New + 2;
                ...
                console.log(points);
                console.log(fittedBoxes);
                const refinedBoxes: number[][] = [];

                const continuousLimit = 10;
                const thresholded = binaryMat;
                for (const box of fittedBoxes) {
                    ...
                    for (let i = y2; i < thresholded.rows; i++) {
                        let sum = 0;
                        for (let x = x1; x < x2; x++) {
                            sum += thresholded.ucharAt(i, x);
                        }
                ...
                const fittedRectangle = refinedBoxes[0];
                console.log(fittedRectangle);
