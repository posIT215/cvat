...
class DrawFitRectControlComponent extends React.PureComponent<Props & DispatchToProps, State> {
    ...
    private onFitting = async (e: Event): Promise<void> => {
        ...
        const { activeLabelID } = this.props || this.state;
        if (!isActivated) {
            return;
        }
        ...
        if (isDone) {
            ...
            if (pressedPoints.length >= 4) {
                const [x1, y1, x2, y2] = pressedPoints.slice(0, 4);

                // Constructing a rectangle using the given points
                const rectanglePoints = [x1, y1, x2, y2];
...
export default connect(mapStateToProps, mapDispatchToProps)(DrawFitRectControlComponent);
